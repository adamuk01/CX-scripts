...
def dump_to_csv(db_file, table_name,fields, category):
    ...
    query = f"SELECT {', '.join(fields)} FROM {table_name} WHERE race_category_current_year LIKE '%{category}%' AND gender LIKE 'Female' ORDER by best_X_points DESC"
    ...
    csv_file = f"{category}-Female-leaguetable.csv"
    ...
    # Execute SELECT query to fetch specific fields and sort the data - Now do the males:
    query = f"SELECT {', '.join(fields)} FROM {table_name} WHERE race_category_current_year LIKE '%{category}%' AND gender = 'Male' ORDER by best_X_points DESC"
    cursor.execute(query)
    ...
    # Write the data to a CSV file
    csv_file = f"{category}-Male-leaguetable.csv"
    with open(csv_file, 'w', newline='') as file:
        writer = csv.writer(file)
