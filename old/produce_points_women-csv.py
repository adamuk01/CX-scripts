...
print("Creating CSV file for points from ",db_file,"for women")
...
def output_race_results(db_file, output_csv):
    ...
    cursor.execute("SELECT race_number, full_name, club_name, race_category, r1_overall_position, r2_overall_position, r3_overall_position, r4_overall_position, r5_overall_position, r6_overall_position, r7_overall_position, r8_overall_position, r9_overall_position, r10_overall_position, r11_overall_position, r12_overall_position FROM riders")
    ...
    for row in rows:
        ...
        club_name = row[2]
        category = row[3]
        rider_points.setdefault((rider_number, rider_name, club_name, category), [0] * 12)  # Initialize points list for each rider
        
        for i, position in enumerate(row[4:], start=0):  # Start from race_position1
            if position is not None:
                rider_points[(rider_number, rider_name, club_name, category)][i] = calculate_points(position)
    ...
    with open(output_csv, 'w', newline='') as csvfile:
        fieldnames = ['Position', 'Rider Number', 'Rider Name', 'Club Name', 'Category'] + [f'Race {i+1}' for i in range(12)] + ['Best 10 Total']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        for idx, rider in enumerate(sorted_riders, start=1):
            rider_number, rider_name, club_name, category = rider
            points = rider_points[rider]
            best_10_total = rider_best_10_points[rider]
            writer.writerow({'Position': idx, 'Rider Number': rider_number, 'Rider Name': rider_name, 'Club Name': club_name, 'Category': category, **{f'Race {i+1}': points[i] for i in range(12)}, 'Best 10 Total': best_10_total})

    conn.close()

# Find Females
output_csv = 'points-women.csv'
output_race_results(db_file, output_csv)
