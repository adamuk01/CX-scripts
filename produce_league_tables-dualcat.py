...
parser.add_argument("cat1", help="Race Category, e.g Under 8 or Under 6 or Vet 50-59 - must be full category in quotes")
parser.add_argument("cat2", help="Race Category, e.g Under 8 or Under 6 or Vet 50-59 - must be full category in quotes")
...
print("Creating CSV file for League tables from ",db_file,"with current values for",category1,"and",category2)

import sqlite3
import csv


def dump_to_csv(db_file, table_name,fields, category1, category2):
    conn = sqlite3.connect(db_file)
    cursor = conn.cursor()

    # Execute SELECT query to fetch specific fields and sort the data - Female first:
    query = f"SELECT firstname || ' ' || surname AS full_name, {', '.join(fields)} FROM {table_name} WHERE race_category_current_year LIKE '%{category1}%' OR race_category_current_year LIKE '%{category2}%' ORDER by best_X_points DESC"
    ...
    csv_file = f"{category1}-{category2}-leaguetable.csv"
    with open(csv_file, 'w', newline='') as file:
        writer = csv.writer(file)

    # Write header row
        writer.writerow(['Position','Full Name', *fields])
...
dump_to_csv(db_file, table_name, fields, category1, category2)
